...
class Snake(tk.Canvas):
    ...
    def create_objects(self):
        for x_position, yposition in self.snake_positions:
            self.create_image(x_position, yposition,
                              image=self.snake_body, tag="snake")

        self.create_image(*self.food_position, image=self.food, tag="food")
        self.create_text(100, 12, text=f"Score: {self.score}, speed: {moves_per_second}", tag="score", fill="#fff", font=(
            "TKDefaultFont", 14))

        self.create_rectangle(7, 27, 593, 613, outline="#512623")
    ...
    def check_food_collision(self):
        if self.snake_positions[0] == self.food_position:
            ...
            self.itemconfigure(
                score, text=f"Score: {self.score}, speed: {moves_per_second}")
            self.food_position = self.set_new_food_position()
    ...
    def end_game(self):
        ...
        self.create_text(self.winfo_width()/2, self.winfo_height()/2,
                         text=f"Game Over\nYou scored {self.score} points.", fill="#fff", font=("TKDefaultFont", 20))
